...
var main=(function(){'use strict';function _inheritsLoose$1(subClass, superClass) {
...
function Autoplay({
  ...
  var _proto = BreakPoints.prototype;

  _proto.setBreakpoints = function setBreakpoints() {
    var _this = this;
    ...
    this.mediaQueryListSm.addEventListener('change', function () {
      _this.breakpointChecker();
    });
    this.mediaQueryListMd.addEventListener('change', function () {
      _this.breakpointCheckerMd();
    });
  };

  _proto.breakpointChecker = function breakpointChecker() {
  ...
  _proto.breakpointCheckerMd = function breakpointCheckerMd() {
  ...
  _proto.onMobile = function onMobile() {};

  _proto.onMediaQueryMd = function onMediaQueryMd() {};

  _proto.onDesktop = function onDesktop() {};
...
SwiperSettings.features = {
  sliderCarousel: {
    mobileVersion: 'carousel'
  }
};var SwiperSlider = function (_BreakPoints) {
  _inheritsLoose$1(SwiperSlider, _BreakPoints);

  function SwiperSlider(node) {
    var _this;

    _this = _BreakPoints.call(this) || this;
    _this.node = node;
    _this.id = _this.node.dataset.swiperId;
    _this.type = _this.node.dataset.swiperSlider;
    _this.params = SwiperSettings.getParams(_this.type);
    _this.features = SwiperSettings.getFeatures(_this.type);
  ...
  _proto.init = function init() {
    var params = this.features.mobileVersion && this.isMobile === true ? this.features.mobileVersion : this.type;
  ...
  SwiperSlider.init = function init() {
    SwiperSlider.item = Array.from(document.querySelectorAll('[data-swiper-slider]')).map(function (node, index) {
      return new SwiperSlider(node, index);
    });
  };

  return SwiperSlider;
}(BreakPoints);function _assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
...
gsapWithCSS.core.Tween;var EventEmitter = function () {
  ...
  _proto.on = function on(_names, callback) {
    var _this = this;
  ...
  _proto.off = function off(_names) {
    var _this2 = this;
        ...
        if (name.namespace === 'base') {
          for (var namespace in _this2.callbacks) {
            if (_this2.callbacks[namespace] instanceof Object && _this2.callbacks[namespace][name.value] instanceof Array) {
              ...
              if (Object.keys(_this2.callbacks[namespace]).length === 0) delete _this2.callbacks[namespace];
            }
          }
  ...
  _proto.trigger = function trigger(_name, _args) {
  ...
  _proto.resolveNames = function resolveNames(_names) {
    var names = _names;
  ...
  _proto.resolveName = function resolveName(name) {
    var newName = {};
  ...
  return EventEmitter;
}();var CarouselNav = function (_EventEmitter) {
  _inheritsLoose$1(CarouselNav, _EventEmitter);

  function CarouselNav(node, items) {
    var _this;

    _this = _EventEmitter.call(this) || this;
    _this.node = node;
    _this.items = items;
    _this.nav = node.querySelector('[data-carousel-nav]');
  ...
  var _proto = CarouselNav.prototype;
  ...
  _proto.addListener = function addListener() {
    ...
    this.buttons.forEach(function (button, index) {
      button.addEventListener('click', function (e) {
        ...
        _this2.setActive(current);

        _this2.trigger('carouselNavClick', [index]);
      });
    });
  };

  _proto.setActive = function setActive(current) {
  ...
  return CarouselNav;
}(EventEmitter);var Carousel = function (_BreakPoints) {
  _inheritsLoose$1(Carousel, _BreakPoints);
  ...
  function Carousel(node) {
    ...
    _this = _BreakPoints.call(this) || this;
    _this.node = node;
    _this.wrapper = _this.node.querySelector('.swiper-wrapper');

    var attribute = _this.node.getAttribute('data-carousel');
  ...
  _proto.destroy = function destroy() {
    ...
    if (this.items && this.items !== null) {
      this.items.forEach(function (item, i) {
        var card = item.querySelector('.carousel__card');
        ...
        item.removeEventListener('mouseenter', _this2.onOver.bind(_this2));
      });
      this.items = null;
    }
  ...
  _proto.setView = function setView() {
    ...
    this.items.forEach(function (item) {
      gsapWithCSS.to(item, {
        scale: _this3.settings.scale,
        duration: _this3.settings.speed,
        ease: _this3.settings.easing
      });
      var card = item.querySelector('.carousel__card');
      var caption = item.querySelector('figcaption');

      _this3.setCardPos(card, item);

      _this3.setCaptionPos(caption, card);
    ...
    if (this.settings.nav) {
      ...
      this.nav.on('carouselNavClick', function (index) {
        _this3.onOver(_this3.items[index]);
      });
    }
  ...
  _proto.addListeners = function addListeners() {
    ...
    this.items.forEach(function (item, i) {
      item.addEventListener('mouseenter', _this4.onOver.bind(_this4));
    });
  };

  _proto.setCardPos = function setCardPos(card, parent, pos) {
    if (!card || !parent) {
      return;
    }
  ...
  _proto.setCaptionPos = function setCaptionPos(caption, card, pos) {
    if (!caption || !card) {
      return;
    }
  ...
  _proto.setWrapperPos = function setWrapperPos(item) {
    var rect = item.getBoundingClientRect();
    gsapWithCSS.to(this.wrapper, {
      x: (this.wrapper.clientWidth / 2 - rect.left + item.clientWidth) * 0.08,
      duration: this.settings.speed,
      ease: this.settings.easing
    });
  };

  _proto.onOver = function onOver(e) {
    var _this5 = this;
    ...
    var el = !e.currentTarget ? e : e.currentTarget;
    var currentId = this.items.indexOf(el);
    var totalLenght = this.items.length;
    var nextvalue = totalLenght - currentId - 1;
    var nextDifference = (1 - this.settings.scale) / nextvalue;
    var prevDifference = (1 - this.settings.scale) / currentId;
    var counter = 1;
    ...
    this.items.forEach(function (item, i) {
      ...
      if (i < currentId) {
        var percent = currentId === 1 ? nextvalue - 1 : i;
        gsapWithCSS.to(item, {
          scale: _this5.settings.scale + prevDifference * percent,
          duration: _this5.settings.speed,
          ease: _this5.settings.easing
        });

        _this5.setCardPos(card, item, 'prev');

        _this5.setCaptionPos(caption, card, 'prev');
        ...
        _this5.setWrapperPos(item);

        _this5.setCardPos(card, item);

        _this5.setCaptionPos(caption, card);

        item.classList.add('active');
        item.style.zIndex = totalLenght;

        if (_this5.settings.nav) {
          _this5.nav.buttons.forEach(function (button) {
            button.classList.remove('active');
          });

          _this5.nav.buttons[i].classList.add('active');
        }
      } else if (i > currentId) {
        gsapWithCSS.to(item, {
        ...
        _this5.setCardPos(card, item, 'next');

        _this5.setCaptionPos(caption, card, 'next');
  ...
  Carousel.init = function init() {
    Carousel.item = Array.from(document.querySelectorAll('[data-carousel]')).map(function (node, index) {
      return new Carousel(node, index);
    });
  };

  return Carousel;
}(BreakPoints);var Main = function (_Page) {
  _inheritsLoose$1(Main, _Page);
